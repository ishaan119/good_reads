...
#   like this:
app.config.update(
    GOODREADS_CONSUMER_KEY='AiavlqI7ZR55oBzDbT1y2w',
    GOODREADS_CONSUMER_SECRET='nyxzFDTt63e8f9SjgXlBOIQylq2eNqrRszbS2TiDzA',
    SECRET_KEY='just a secret key, to confound the bad guys',
    DEBUG = True
)


# setup the goodreads endpoint
goodreads = RauthOAuth1(
    name='goodreads',
...
def index():
    ...
    if g.user is not None:
        resp = goodreads.get('statuses/home_timeline.json')
        if resp.status == 200:
            tweets = resp.content
        else:
...
def tweet():
    '''
    Calls the remote goodreads API to create a new status update.
    '''
    if g.user is None:
    ...
    resp = goodreads.post('statuses/update.json', data={
        'status': status
    })
    if resp.status == 403:
...
def login():
    ...
    '''
    return goodreads.authorize(callback=url_for('authorized',
        _external=True,
